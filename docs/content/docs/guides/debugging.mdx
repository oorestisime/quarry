...
These docs include development APIs beyond npm 0.9.1.
...
| Unmatched LEFT JOIN values are zero or empty strings | Ordinary joins use ClickHouse defaults; development `leftJoinNullable()` opts into nulls |
| Only one result is needed | Development first-row helpers add LIMIT 1; on 0.9.1, call `.limit(1)` explicitly |
