...
Quarry never auto-deserializes query results into JavaScript `Date` objects.
Selected `Date`, `DateTime`, and `DateTime64` values still come back as
`string` under the default JSON formats.

When you bind a raw JavaScript `Date` through `param(...)`, Quarry formats it
in UTC based on the target ClickHouse type:

| Target type | Normalized format |
| --- | --- |
| `Date` / `Date32` | `YYYY-MM-DD` |
| `DateTime` | `YYYY-MM-DD HH:mm:ss` |
| `DateTime64(p)` | `YYYY-MM-DD HH:mm:ss.<fraction>` |

When Quarry cannot see the target column type, a bare JavaScript `Date`
defaults to a `DateTime` placeholder. Use `param(date, "Date")`,
`param(date, "DateTime")`, or `param(date, "DateTime64(3)")` when you want
exact control over the bound ClickHouse type.
...
await db.insertInto("events").values([
  {
    created_at: new Date("2025-01-01T12:34:56.789Z"),
    event_type: "signup",
  },
]);
...
db
  ...
  .where("e.created_at", ">=", new Date("2025-01-01T00:00:00Z"));
...
db
  ...
  .where(
    "e.created_at",
    ">=",
    param(new Date("2025-01-01T00:00:00Z"), "DateTime64(3)"),
  );
```

<Callout type="info">
JavaScript `Date` only carries millisecond precision. If you bind a
`DateTime64(p)` with `p > 3`, Quarry zero-pads the extra fractional digits.
</Callout>
...
| `eb.fn.now64(precision)`              | Current query-time `DateTime64(precision)`.                      |
...
| `eb.fn.toYear(value)`                 | Year as integer.                                                 |
| `eb.fn.toMonth(value)`                | Month as integer (1–12).                                         |
...
const rows = await db
  ...
  .selectExpr((eb) => [
    ...
    eb.fn.greatest(eb.ref("u.id"), eb.val(0)).as("min_id"),
...
reach for in analytics queries: `count`, `countDistinct`, `sum`, `avg`, `min`,
`max`, `uniq`, `uniqExact`, `groupArray`, `any`, `anyLast`, plus their `-If`
combinators. See the [`ExpressionBuilder` reference](/docs/reference/expressions#aggregates)
...
const stats = await db
  ...
  .selectExpr((eb) => [
    ...
    eb.fn.countDistinct("e.session_id").as("unique_sessions"),
    eb.fn.sum("e.amount").as("total_amount"),
    eb.fn.avg("e.amount").as("avg_amount"),
    eb.fn.uniqExact("e.session_id").as("unique_sessions_exact"),
  ])
  .groupBy("e.user_id")
  .execute();
```
...
## Reusing expressions externally

If you build predicates in a helper function or an external package, you can
return an `Expression` object and pass it directly to `where`, `prewhere`, or
`having` instead of using a callback:

```ts
import { ExpressionBuilder } from "quarry";

function buildVipPredicate(eb: ExpressionBuilder<any>) {
  return eb.and([
    eb.fn.has("u.tags", "premium"),
    eb.cmp("u.status", "=", "active"),
  ]);
}

const eb = new ExpressionBuilder<any>();
const vipPredicate = buildVipPredicate(eb);
...
Multiple `.where(expression)` calls are combined with `AND`, just like the
callback and positional forms.
