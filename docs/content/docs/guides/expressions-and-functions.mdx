...
`string` under the default JSON formats. The automatic behavior is on the
input side.

In schema-backed operations, Quarry formats raw JavaScript `Date` values in UTC
based on the target ClickHouse type:
...
That automatic formatting applies to:

- schema-backed `insertInto(...).values(...)`
- schema-backed predicates such as `.where("e.created_at", ">=", new Date(...))`
