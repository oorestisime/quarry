...
Wraps a nonempty tuple of expressions in a logical `AND`. TypeScript rejects
empty arrays and general arrays whose length is unknown. JavaScript callers get
an immediate error if the array is empty.
...
Same nonempty tuple requirement, logical `OR`. An empty array throws immediately.
...
The tables below show the value type parameter of `Expression`. Helpers also
carry a separate predicate input type; for example, a date expression reads as
`string` while accepting `string | Date` in predicates.
...
`avg` and `avgIf` return `number | null` because empty results serialize as null;
`min` / `max` / `any` / `anyLast` / `argMin` / `argMax` preserve the input type; `groupArray`
preserves the input type except that `NULL` elements are dropped from nullable
inputs. Numeric `quantile` returns `number | null` for the same empty-input reason.
...
| `eb.fn.countDistinct(value)`        | `Expression<string>`             | `count(DISTINCT value)`. Respects `count_distinct_implementation`. |
...
| `eb.fn.avg(value)`                  | `Expression<number \| null>`     | Empty input serializes as `null`.                                |
| `eb.fn.avgIf(value, condition)`     | `Expression<number \| null>`     | `null` when no values contribute.                               |
...
| `eb.fn.quantile(level, value)`      | `Expression<number \| null>`     | Approximate numeric quantile; empty input is `null`. `level` must be between `0` and `1`. |
...
| `eb.fn.argMin(value, by)` | `Expression<T>`      | Returns `value` from the row where `by` is smallest.                  |
| `eb.fn.argMax(value, by)` | `Expression<T>`      | Returns `value` from the row where `by` is largest.                   |
...
The table shows results for non-nullable inputs. A nullable column or expression
adds `null` to the result: `toUInt32` of `string | null` yields
`Expression<number | null>`. This is preserved through aliases and subqueries.
...
| `eb.fn.toUInt8(value)`                 | `Expression<number>` | `UInt8`                  |
...
| `eb.fn.now64(precision)`               | `Expression<string>` | Current `DateTime64(precision)`. Precision must be `0..9`.     |
...
| `eb.fn.toYear(value)`                  | `Expression<number>` | Year as `UInt16`.                                              |
| `eb.fn.toMonth(value)`                 | `Expression<number>` | Month as `UInt8` (1–12).                                       |
...
| Helper                             | Returns                | Notes                                                                              |
| ---------------------------------- | ---------------------- | ---------------------------------------------------------------------------------- |
| `eb.fn.if(condition, then, else)`  | `Expression<Then \| Else>` | ClickHouse `if`. Return type is the union of the two branch types.              |

```ts
db
  .selectFrom("users as u")
  .selectExpr((eb) => [
    eb.fn.if(eb.cmp("u.status", "=", "active"), eb.ref("u.label"), eb.val("inactive")).as("status_label"),
  ]);
```
...
| Helper                        | Returns              | Notes                                                   |
| ----------------------------- | -------------------- | ------------------------------------------------------- |
| `eb.fn.least(...values)`      | `Expression<T>`      | Minimum of one or more values. Preserves input type.    |
| `eb.fn.greatest(...values)`   | `Expression<T>`      | Maximum of one or more values. Preserves input type.    |
| `eb.fn.ceil(value)`           | `Expression<T>`      | Rounds up. Preserves the resolved input type.           |
| `eb.fn.floor(value)`          | `Expression<T>`      | Rounds down. Preserves the resolved input type.         |

```ts
db
  .selectFrom("users as u")
  .selectExpr((eb) => [
    eb.fn.least(eb.ref("u.id"), eb.val(10)).as("capped_id"),
    eb.fn.ceil("u.amount").as("amount_ceil"),
    eb.fn.floor("u.amount").as("amount_floor"),
  ]);
```
...
Dictionary functions require the dictionary to be declared in the schema with
`TypedDictionary<Row>`. The `dictName` and `attrName` arguments are then
type-checked at compile time, and the return type is inferred from the
schema.

| Helper                                                        | Returns                                  |
| ------------------------------------------------------------- | ---------------------------------------- |
| `eb.fn.dictGet(dict, attr, key)`                              | `Expression<AttrType>`                   |
| `eb.fn.dictGetOrDefault(dict, attr, key, default)`            | `Expression<AttrType>`                   |
| `eb.fn.dictHas(dict, key)`                                    | `Expression<number>`                     |

The `key` argument can be a single column reference or an array of column
references for composite keys (compiled to `tuple(...)` in SQL).

For `RANGE_HASHED` dictionaries, an optional `rangeDate` argument provides the
date or datetime for the range lookup:

```ts
interface DB {
  events: { partner_id: number; event_date: string };
...
db.selectFrom("events as e").selectExpr((eb) => [
  ...
  eb.fn.dictGet("partner_rates", "rate_cents", ["e.partner_id", "e.event_date"]).as("composite_rate"),
  // RANGE_HASHED lookup with date
  eb.fn.dictGet("partner_rates", "rate_cents", "e.partner_id", "e.event_date").as("rate_at_date"),
  // Lookup with fallback
  eb.fn.dictGetOrDefault("partner_rates", "currency", "e.partner_id", "USD").as("currency"),
  // RANGE_HASHED lookup with fallback and date
  eb.fn.dictGetOrDefault("partner_rates", "currency", "e.partner_id", "USD", "e.event_date").as("currency_at_date"),
  // Check existence
  eb.fn.dictHas("partner_rates", "e.partner_id").as("has_rate"),
  // RANGE_HASHED existence check with date
  eb.fn.dictHas("partner_rates", "e.partner_id", "e.event_date").as("has_rate_at_date"),
]);
```

`dictGetOrDefault` type-checks the `default` value against the schema attribute
type, so a mismatch (e.g. passing `"USD"` for a `number` attribute) produces a
compile error.
