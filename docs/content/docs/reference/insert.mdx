...
Call `columns(...)` before `values(...)` or `fromSelect(...)`.
...
Quarry normalizes bound query parameter values when it has an explicit
ClickHouse type, but `insertInto(...).values(...)` rows are otherwise sent as
you provide them.

If you need exact ClickHouse date/time formatting in inserts, pass explicit
strings or use `INSERT INTO ... SELECT` with typed expressions.
...
ClickHouse maps source values to target columns by **position**. Quarry requires
explicit `.columns(...)` and explicit source selections, and checks the ordered
types and column count. Aliases do not change insertion order; selecting
`email, id` for targets `id, email` fails typechecking when their types differ.
Columns with the same TypeScript type still require care: types cannot distinguish
two different string columns. Table stars are rejected because their order is
not represented by the schema interface.
