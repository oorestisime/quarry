...
```ts
import { ExpressionBuilder } from "quarry";

const eb = new ExpressionBuilder<any>();
const isPremium = eb.fn.has("u.tags", "premium");

db.selectFrom("users as u").select("u.id").where(isPremium);
```
...
`having` accepts the same callback overloads as `where`, including the
...
Adds `WITH TOTALS` to `GROUP BY`. A grouped query is required. Because
ClickHouse returns totals separately in the `JSON` response document, execute
this query with [`executeWithTotals()`](#executewithtotalsoptions), not
`execute()`.
...
### `arrayJoin(column)` / `leftArrayJoin(column)`

Expands a typed array column into one row per element. `arrayJoin` removes rows
whose array is empty; `leftArrayJoin` preserves them with ClickHouse's default
element value.
...
const tags = await db
  ...
  .select("e.id", "e.tags")
  .execute();
// e.tags is string here, not string[]
```

Call the array-join method before `select` or `selectExpr`. This lets Quarry
change that column from `T[]` to `T` before it infers the result type. Multiple
array joins can be chained, including joins over nested arrays.
...
use `leftJoinNullable()` instead of `leftJoin()`. See
...
### `limitBy(limit, ...expressions)`

Limits rows per distinct expression combination before the query-wide
`limit(...)`. Pass `{ limit, offset }` instead of a number to skip rows inside
each group.
...
db
  .selectFrom("events as e")
  .selectAll()
  .limitBy({ limit: 2, offset: 1 }, "e.user_id");
```

Each expression is either a scoped column ref or an expression-builder
callback. At least one is required, and both values must be non-negative
integers.
...
### `executeWithTotals(options?)`

Runs a query configured with `withTotals()` using ClickHouse's `JSON` document
format and returns `{ rows, totals }`. Both fields use the query's inferred
output type. Calling this method without `withTotals()` throws; likewise,
calling `execute()` on a totals query throws rather than silently dropping the
separate totals object.

### `stream(options?)`

Returns an `AsyncIterable<Output>` backed by the driver's `JSONEachRow` stream.
Rows are decoded and yielded chunk by chunk, so the full result is never held
in memory.
...
The initial driver `query()` call follows the configured retry policy. Errors
after streaming has begun are not retried because doing so could yield duplicate
rows. Streaming totals queries is not supported; use `executeWithTotals()`.

### `executeTakeFirst(options?)`

Executes with an overall `LIMIT 1` and returns the first row, or `undefined` if
there were none. An explicit `LIMIT 0` remains zero, and any offset is preserved.
The original builder is unchanged. Filters run in ClickHouse; only the limited
result is downloaded and decoded. Use `orderBy()` when the choice of first row
...
`SelectQueryBuilder<Sources, Scope, Output, OutputColumns>` carries four parameters that
...
| `OutputColumns` | Internal ClickHouse column metadata retained when a query becomes a subquery. |
