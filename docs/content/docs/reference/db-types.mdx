...
```ts
import type { TypedView } from "quarry";
...
```ts
import type { TypedTable } from "quarry";
...
```ts
import type { TypedDictionary } from "quarry";
...
```ts
import type { ColumnType } from "quarry";
...
The CLI introspection flow uses these helpers automatically. Native `JSON`
columns, including `JSON(...)` with options or typed paths, default to
`Record<string, unknown>` for reads and inserts.
...
the [introspection guide](/docs/guides/introspection#type-overrides).
