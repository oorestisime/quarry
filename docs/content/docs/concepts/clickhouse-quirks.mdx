...
| `GROUP BY`                  | ✅ | Column refs only (not aliases) |
...
### Inserts only via `JSONEachRow`

Symmetrically, inserts always use `format: "JSONEachRow"`. Your row objects
are sent to the driver's `insert()` method directly &mdash; they never get
serialized into the SQL query. This is what makes large batch inserts
efficient. The downside is that you cannot use Quarry to do an
`INSERT INTO ... SELECT FROM` query yet.
